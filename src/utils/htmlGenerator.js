...
const tokens_1 = require("../analizador/tokens");
class HtmlGenerator {
    ...
    buildPensumStructure(tokens) {
        ...
        for (let i = 0; i < tokens.length; i++) {
            ...
            if (token.tipo === tokens_1.TOKEN_TYPES.PALABRA_RESERVADA) {
                switch (token.lexema) {
                    case tokens_1.KEYWORDS.CARRERA:
                        if (currentCarrera) {
                        ...
                        break;
                    case tokens_1.KEYWORDS.SEMESTRE:
                        if (currentCarrera) {
                            if (currentSemestre) {
                    ...
                    case tokens_1.KEYWORDS.CURSO:
                        ...
                        break;
                    case tokens_1.KEYWORDS.NOMBRE:
                        if (currentCurso)
                            expecting = 'curso_nombre';
                        break;
                    case tokens_1.KEYWORDS.AREA:
                        if (currentCurso)
                            expecting = 'curso_area';
                        break;
                    case tokens_1.KEYWORDS.PRERREQUISITOS:
                        if (currentCurso)
                            expecting = 'prerrequisitos_lista';
                        break;
                }
            }
            else if (token.tipo === tokens_1.TOKEN_TYPES.CADENA_TEXTO) {
            ...
            else if (token.tipo === tokens_1.TOKEN_TYPES.NUMERO_ENTERO) {
                ...
                if (expecting === 'semestre_numero' && currentSemestre) {
                    ...
                    let j = i;
                    while (tokens[j + 1] && tokens[j + 1].tipo !== tokens_1.TOKEN_TYPES.PARENTESIS_CERRAR) {
                        j++;
                        if (tokens[j].tipo === tokens_1.TOKEN_TYPES.NUMERO_ENTERO) {
                            currentCurso.prerequisitos.push(parseInt(tokens[j].lexema));
                        }
                        else if (tokens[j].tipo !== tokens_1.TOKEN_TYPES.COMA) {
            ...
            else if (token.tipo === tokens_1.TOKEN_TYPES.LLAVE_CERRAR) {
