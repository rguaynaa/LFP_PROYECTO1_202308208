...
import { Carrera, Semestre, Curso } from '../models/carrera';
import { KEYWORDS, TOKEN_TYPES } from '../analizador/tokens';

export class HtmlGenerator {

    generateTokensHtml(tokens: Token[]): string {
        let htmlContent = `
            <!DOCTYPE html>
            <html lang="en">
            <head>
    ...
    private buildPensumStructure(tokens: Token[]): Carrera[] {
        const carreras: Carrera[] = [];
        let currentCarrera: Carrera | null = null;
        let currentSemestre: Semestre | null = null;
        ...
        let expecting: 'carrera_nombre' | 'semestre_numero' | 'curso_codigo' | 'curso_nombre' | 'curso_area' | 'prerrequisitos_lista' | null = null;
            ...
            if (token.tipo === TOKEN_TYPES.PALABRA_RESERVADA) {
                switch (token.lexema) {
                    ...
                    case KEYWORDS.NOMBRE:
                        if (currentCurso) expecting = 'curso_nombre';
                        break;
                    case KEYWORDS.AREA:
                        if (currentCurso) expecting = 'curso_area';
                        break;
                    case KEYWORDS.PRERREQUISITOS:
                        if (currentCurso) expecting = 'prerrequisitos_lista';
                        break;
                }
    ...
    generatePensumHtml(tokens: Token[]): string {
